...
class MarketFromFile:
    def __init__(self, symbol_config, date):
        ...
        with trades_reader(date, symbol_config) as tr:
            for trade in tr:
        ...
        with stats_reader(date, symbol_config) as sr:
            for stat in sr:
...
def run():
    ...
    for symbol in startegy_engine.root.get_symbols():
        ...
        market = MarketFromFile(symbol_config, config.date)
